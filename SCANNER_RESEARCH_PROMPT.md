...
5. Insider activity (SEC Form 4, last 90 days): open-market cluster BUYING by multiple insiders = confirmation worth noting; heavy insider SELLING into the current move = red flag. Ignore routine 10b5-1 plan sales and single small trades — clusters and unusual size matter, noise doesn't. Politician (STOCK Act) disclosures are context only; their 30-45 day lag makes them useless for timing.
6. Setup quality: clean breakout / pullback candidate / too extended / failed move.
7. Clear invalidation level or event. If no clear invalidation exists, say Reject.
8. What would make me wait instead of chase.
9. For Deep dive verdicts only: a draft proposal card per SETUPS.md — usually a
