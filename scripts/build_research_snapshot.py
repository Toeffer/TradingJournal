...
# Dated, immutable copies for manifest verification. The live OUTPUT_CSV is
# regenerated by the research-snapshot workflow whenever scanner data changes,
# so a manifest hash recorded against it goes stale within hours; manifests
# stay verifiable against the archive instead.
ARCHIVE_DIR = REPO_ROOT / "research/snapshots"
...
def archive_snapshot(content: bytes, *, now: datetime | None = None) -> Path:
    """Write a dated, immutable copy of the snapshot for manifest verification.

    Existing archives are never overwritten: a same-day archive with identical
    content is reused, and a same-day archive with different content gets a
    -HHMM suffix (mirroring how a second same-day candidate report is named).
    """
    now = now or datetime.now(timezone.utc)
    ARCHIVE_DIR.mkdir(parents=True, exist_ok=True)
    dated = ARCHIVE_DIR / f"research_snapshot-{now:%Y-%m-%d}.csv"
    if dated.exists():
        if dated.read_bytes() == content:
            return dated
        dated = ARCHIVE_DIR / f"research_snapshot-{now:%Y-%m-%d}-{now:%H%M}.csv"
        if dated.exists() and dated.read_bytes() == content:
            return dated
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Also write an immutable dated copy to research/snapshots/ so candidate "
            "manifests stay verifiable after the live snapshot is refreshed. Use this "
            "before every research run."
        ),
    )
    ...
    if args.archive:
        archived = archive_snapshot(content)
        print(f"Archived {archived.relative_to(REPO_ROOT)} for manifest verification.")
    return 0
