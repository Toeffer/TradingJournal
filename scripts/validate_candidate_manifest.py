...
# Dated, immutable copies written by `build_research_snapshot.py --archive`.
# The live data/research_snapshot.csv is regenerated by automation whenever
...
def archived_snapshot_matches(digest: str, *, root: Path = REPO_ROOT) -> bool:
    """True when any archived snapshot's SHA-256 equals `digest`."""
    archive_dir = root / SNAPSHOT_ARCHIVE_SUBDIR
        ...
        if not snapshot_path.exists():
            ...
            if actual != digest and not archived_snapshot_matches(digest, root=root):
                errors.append(
                    ...
                    f"`{SNAPSHOT_ARCHIVE_SUBDIR}/`. The live snapshot is regenerated "
                    "by automation; research runs must archive the snapshot they "
                    "used via `python scripts/build_research_snapshot.py --archive`"
                )
